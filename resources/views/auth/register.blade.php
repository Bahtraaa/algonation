...
@section('content')
    <div class="mx-auto grid min-h-[80vh] max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-2 lg:px-8">
        ...
        <div class="animate-fade-up order-2 lg:order-1">
            <div class="mx-auto w-full max-w-md">
                <div class="mb-8 text-center lg:text-left">
                    ...
                    <h1 class="font-display text-3xl font-extrabold">Buat Akun Baru</h1>
                    <p class="mt-2 text-sm text-slate-500">Daftar gratis dan mulai belanja fashion pilihanmu sekarang.</p>
                </div>
                ...
                <div class="glass-strong rounded-2xl p-6 sm:p-8">
                    <form method="POST" action="{{ route('register.store') }}" class="space-y-5">
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <div>
                                <label for="username" class="label">Username <span class="text-slate-400">(opsional)</span></label>
                                <input id="username" type="text" name="username" value="{{ old('username') }}"
                                    class="input @error('username') border-rose-400 @enderror" placeholder="username">
                                @error('username')
                                    <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div x-data="{ show: false }">
                            ...
                            <div class="relative">
                                <input id="password_confirmation" :type="show ? 'text' : 'password'" name="password_confirmation" required
                                    class="input pr-12" placeholder="Ulangi password">
                                <button type="button" @click="show = !show"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                                    <svg x-show="!show" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    <svg x-show="show" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                        ...
                        <button type="submit" class="btn-primary w-full btn-lg">Daftar Sekarang</button>
                    </form>

                    <p class="mt-6 text-center text-sm text-slate-500">
                        Sudah punya akun?
                        <a href="{{ route('login') }}" class="font-semibold text-primary hover:underline dark:text-primary-soft">Masuk di sini</a>
                    </p>
                </div>
            </div>
        </div>
        ...
        {{-- Banner --}}
        <div class="relative hidden lg:block order-1 lg:order-2">
            <div class="animate-float">
                <div class="relative mx-auto max-w-md">
                    <div class="absolute -inset-6 rounded-[2.5rem] bg-gradient-to-br from-[#5c3d25] via-primary to-[#4a2f1b] opacity-30 blur-3xl"></div>
                    <div class="glass-strong relative overflow-hidden rounded-[2rem] p-8">
                        <span class="badge-primary mb-4">Mengapa bergabung?</span>
                        <h2 class="font-display text-2xl font-extrabold leading-snug">Belanja fashion premium, nyaman, dan gaya.</h2>
